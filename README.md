...
- удаление любых ссылок для не-админов
- опциональный лимит сообщений в сутки на пользователя (`0` = выключен, по `Europe/Moscow`)
- лимит `1` фото-сообщение в час на пользователя с эскалацией в `mute 3h`
- удаление слишком длинных текстов с предупреждением пользователя
- дубликаты (включая повторы между чатами): не чаще 1 раза в 12ч, текстовые дубли считаются от 12 символов, одинаковые пересланные тоже учитываются; пересланное фото исключено из проверок дублей; эскалация `объяснение -> предупреждение -> кик`
- антиспам `3 сообщения / 10 сек` с эскалацией `warn -> mute 1h -> ban 24h`
- ночной режим для городских чатов: `23:00-07:00` по локальному времени чата, `1` удаление с объяснением, `2` тихий мут до 07:00
- кросс-чатовое автоисключение пользователей с подтверждённой спам-репутацией
...
- `/allowdomain_list`
- `/set_limit <0..10000>` (`0` = без суточного лимита)
